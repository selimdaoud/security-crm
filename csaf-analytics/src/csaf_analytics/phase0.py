...
.signal-cves::before{{content:"Related CVEs";font-family:inherit;font-size:9px;font-weight:600;line-height:1.4}}.epss-value{{display:inline-flex;align-items:center;justify-content:flex-end;gap:5px}}.epss-trend{{display:inline-grid;place-items:center;width:21px;height:21px;padding:0;border:1px solid var(--blue);border-radius:4px;background:#fff;color:var(--blue);cursor:pointer;font:14px/1 inherit}}.epss-trend:hover{{background:#e8f2fb}}.epss-trend:focus-visible{{outline:2px solid var(--blue);outline-offset:2px}}.epss-trend-dialog{{width:min(720px,94vw)!important}}.epss-trend-dialog .dialog-body{{min-height:240px}}.epss-trend-summary{{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 12px;color:var(--muted)}}.epss-trend-chart{{width:100%;height:auto;min-height:230px;display:block}}.epss-trend-status{{padding:28px 0;text-align:center;color:var(--muted)}}.epss-trend-source{{margin:10px 0 0;color:var(--muted);font-size:11px}}
...
const epssTrendButton=cve=>`<button class="epss-trend" type="button" data-epss-trend-cve="${{esc(cve)}}" title="View 30-day EPSS trend from FIRST" aria-label="View 30-day EPSS trend for ${{esc(cve)}}">↗</button>`;
const epssCell=r=>r.epss==null?"n/a":`<span class="epss-value">${{pct(r.epss)}}${{epssTrendButton(r.cve)}}</span>`;
...
const rows=d.cves.map(r=>`<tr data-priority="${{r.priority_code??"vex"}}" data-kev="${{r.kev===true?1:0}}" data-exploit="${{(r.public_exploits||0)>0?1:0}}" data-pre="${{r.pre_auth?1:0}}" data-vex="${{r.vex_state}}"><td data-sort="${{r.tier??99}}">${{priorityBadge(r)}}</td><td class="mono">${{esc(r.cve)}}</td><td>${{esc(r.decision_reason)}}</td><td class="num" data-sort="${{r.epss??-1}}">${{epssCell(r)}}</td><td class="num" data-sort="${{r.cvss_score??-1}}">${{score(r.cvss_score)}}</td><td class="num" data-sort="${{r.product_count}}">${{n(r.product_count)}}</td><td>${{remediationLinks(r)}}</td><td>${{esc(r.third_party_component)}}</td><td>${{tags(r)}}</td><td>${{esc(r.families.join(", "))}}</td></tr>`).join("");
...
const epssTable=simpleTable(["CVE","EPSS","Percentile","CVSS","Pre-auth","Products"],epss.map(r=>`<tr><td class="mono">${{esc(r.cve)}}</td><td class="num" data-sort="${{r.epss}}">${{epssCell(r)}}</td><td class="num">${{pct(r.epss_percentile)}}</td><td class="num">${{score(r.cvss_score)}}</td><td>${{r.pre_auth?"yes":""}}</td><td class="num">${{n(r.product_count)}}</td></tr>`));
...
root.insertAdjacentHTML("beforeend",`<dialog class="product-detail epss-trend-dialog" id="epss-trend-dialog" aria-labelledby="epss-trend-title"><div class="dialog-head"><div><h2 id="epss-trend-title">EPSS trend</h2><p data-epss-trend-subtitle>Last 30 days, published by FIRST</p></div><button class="drill" type="button" data-epss-trend-close>Close</button></div><div class="dialog-body" data-epss-trend-body></div></dialog>`);
const epssTrendDialog=root.querySelector("#epss-trend-dialog"),epssTrendBody=epssTrendDialog.querySelector("[data-epss-trend-body]");
const epssHistoryPoints=payload=>{{const entry=Array.isArray(payload?.data)?payload.data[0]:payload?.data||payload;const candidates=[entry?.["time-series"],entry?.time_series,entry?.history,payload?.["time-series"],payload?.time_series,payload?.history];const series=candidates.find(Array.isArray)||[];return series.map(point=>({{date:String(point?.date||point?.created||""),epss:Number(point?.epss)}})).filter(point=>point.date&&Number.isFinite(point.epss)).sort((a,b)=>a.date.localeCompare(b.date))}};
const epssTrendChart=points=>{{const width=650,height=260,pad={{top:18,right:22,bottom:38,left:54}},min=Math.min(...points.map(p=>p.epss)),max=Math.max(...points.map(p=>p.epss)),span=Math.max(max-min,0.001),x=index=>pad.left+(width-pad.left-pad.right)*(points.length===1?0:index/(points.length-1)),y=value=>pad.top+(height-pad.top-pad.bottom)*(1-(value-min)/span),path=points.map((p,index)=>`${{index?"L":"M"}}${{x(index).toFixed(1)}},${{y(p.epss).toFixed(1)}}`).join(" "),label=value=>(value*100).toFixed(2)+"%",ticks=[0,.5,1].map(f=>{{const value=min+span*f,py=y(value);return `<g><line x1="${{pad.left}}" x2="${{width-pad.right}}" y1="${{py}}" y2="${{py}}" stroke="#e4e1dc"/><text x="${{pad.left-8}}" y="${{py+4}}" text-anchor="end" fill="#665f58" font-size="11">${{esc(label(value))}}</text></g>`}}).join(""),dates=[0,Math.floor((points.length-1)/2),points.length-1].map(index=>`<text x="${{x(index)}}" y="${{height-12}}" text-anchor="middle" fill="#665f58" font-size="11">${{esc(points[index].date)}}</text>`).join("");return `<svg class="epss-trend-chart" viewBox="0 0 ${{width}} ${{height}}" role="img" aria-label="30-day EPSS score trend">${{ticks}}<path d="${{path}}" fill="none" stroke="#0572ce" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>${{points.map((p,index)=>`<circle cx="${{x(index)}}" cy="${{y(p.epss)}}" r="3.5" fill="#0572ce"><title>${{esc(p.date+": "+label(p.epss))}}</title></circle>`).join("")}}${{dates}}</svg>`}};
const showEpssTrend=async cve=>{{epssTrendDialog.querySelector("[data-epss-trend-subtitle]").textContent=`${{cve}} · last 30 days published by FIRST`;epssTrendBody.innerHTML='<div class="epss-trend-status">Loading EPSS history from FIRST…</div>';typeof epssTrendDialog.showModal==="function"?epssTrendDialog.showModal():epssTrendDialog.setAttribute("open","");try{{const url="https://api.first.org/data/v1/epss?scope=time-series&cve="+encodeURIComponent(cve),response=await fetch(url,{{headers:{{Accept:"application/json"}}}});if(!response.ok)throw new Error("FIRST returned "+response.status);const points=epssHistoryPoints(await response.json());if(!points.length)throw new Error("No time-series points returned");const latest=points.at(-1),first=points[0],change=latest.epss-first.epss;epssTrendBody.innerHTML=`<div class="epss-trend-summary"><span class="badge">${{n(points.length)}} daily scores</span><span class="badge">Latest: ${{pct(latest.epss)}}</span><span class="badge ${{change>=0?"warn":"ok"}}">30-day change: ${{change>=0?"+":""}}${{(change*100).toFixed(2)}} pts</span></div>${{epssTrendChart(points)}}<p class="epss-trend-source">Source: <a href="${{esc(url)}}" target="_blank" rel="noopener">FIRST EPSS API ↗</a>. Hover a point for its daily score.</p>`}}catch(error){{epssTrendBody.innerHTML=`<div class="epss-trend-status">Unable to load the EPSS trend. <a href="https://api.first.org/data/v1/epss?scope=time-series&amp;cve=${{esc(cve)}}" target="_blank" rel="noopener">Open it in FIRST ↗</a><br><small>${{esc(error?.message||"Unknown error")}}</small></div>`}}}};
root.querySelectorAll("[data-epss-trend-cve]").forEach(button=>button.addEventListener("click",()=>showEpssTrend(button.dataset.epssTrendCve)));epssTrendDialog.querySelector("[data-epss-trend-close]").addEventListener("click",()=>{{typeof epssTrendDialog.close==="function"?epssTrendDialog.close():epssTrendDialog.removeAttribute("open")}});epssTrendDialog.addEventListener("click",event=>{{if(event.target===epssTrendDialog&&typeof epssTrendDialog.close==="function")epssTrendDialog.close()}});
